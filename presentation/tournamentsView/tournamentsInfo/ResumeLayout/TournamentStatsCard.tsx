...
const getStatConfig = (label: string) => {
  ...
  if (norm.includes('destacado') || norm.includes('jugador') || norm.includes('mvp')) {
    return { icon: 'trophy-outline' as const, color: '#FBBF24' };
  }
  if (norm.includes('avance') || norm.includes('progreso') || norm.includes('porcent')) {
    return { icon: 'speedometer-outline' as const, color: '#4ADE80' };
  }
  if (norm.includes('amarilla')) {
    return { icon: 'square' as const, color: '#FBBF24' };
  }
  if (norm.includes('roja')) {
    return { icon: 'square' as const, color: '#EF4444' };
  }
